...
export default function Todo({ todo }) {
  ...
  return (
    <li
      ...
      key={todo._id}
    >
      <input className="mx-2 cursor-pointer" type="checkbox" checked={todo.isCompleted} onChange={handleToggle} />
      {/*if todo is done, cross it out and turn it gray*/}
      <p className={`text-lg mx-2 ${todo.isCompleted ? 'line-through text-gray-500' : ''}`}>{todo.text}</p>
      <p className={`text-gray-400 mr-2`}>
        {/*if todo is done, show completedTime
					if not done, show due date */}
        {todo.isCompleted ? `Done ${dayjs(completedTime).format('MMM D, YYYY')}` : `Due ${dayjs(todo.dueDate).format('MMM D, YYYY')}`}
      </p>
      <button
        className="mx-2"
        onClick={e => {
